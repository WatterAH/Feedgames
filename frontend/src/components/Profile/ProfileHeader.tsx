...
const ProfileHeader: React.FC<Props> = ({ data }) => {
  ...
  return (
    <header className="flex flex-col gap-y-4 w-full p-3 border-b">
      <div className="flex flex-row justify-between">
        <div className="namesContainer flex flex-row items-center gap-x-3">
          <ProfilePicture src={pfp} w="h-24" h="h-24" />
          <section className="flex flex-col gap-y-1">
            <span className="flex flex-row items-center gap-x-1">
              <h1 className="text-3xl font-inter font-semibold">{username}</h1>
              {followers > 2 && <CheckBadgeIcon className="text-sky-400 h-8" />}
            </span>
            <p className="font-inter text-gray-500">{name}</p>
          </section>
        </div>
        <button>
          <Options Icon_options={Bars3Icon} className="h-7" options={options} />
        </button>
      </div>
