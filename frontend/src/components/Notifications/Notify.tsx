...
const Notify: React.FC<Props> = ({ data, isLast }) => {
  ...
  return (
    ...
    <div className="flex w-full border-b dark:border-outline p-4 items-center justify-between">
      <div className="flex items-center gap-x-3">
        ...
        <section className="flex flex-col">
          <span className="flex items-center gap-x-1">
            <Link href={`/u/${userId}`}>
              <p className="font-semibold hover:underline text-sm dark:text-white">
                {username}
              </p>
            </Link>
            <p className="text-icon text-sm">{date}</p>
          </span>
          <p className="text-threads dark:text-gray-200 text-sm">{text}</p>
        </section>
      </div>
      <section>
        <Dropdown
