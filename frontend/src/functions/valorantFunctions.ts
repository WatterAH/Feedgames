...
import { Match, PlayerInGame, queueId } from "../interfaces/Valorant";
...
export const getKD = (kills: number, assists: number, deaths: number) => {
...
export const hsPercentagePerRound = (
  bodyShots: number,
  legShots: number,
  headShots: number
) => {
  ...
  return (headShots / totalShots) * 100;
};

export const overallHsPercentage = (match: Match) => {
  let totalShots = 0;
  let totalHs = 0;
};

export const valMatchStats = (match: Match) => {
  const {
    riotUser: { gameName, tagLine },
  } = useRiot();
  ...
  return {
    ...
    characterId,
    stats,
    kda: getKD(stats.kills, stats.assists, stats.deaths),
